...
bool DeviceResources::create_window(HINSTANCE hInstance, int nCmdShow, int width, int height)
{
    ...
    windowSize.width = (width > 0) ? width : defaultWindowWidth;
    windowSize.height = (height > 0) ? height : defaultWindowHeight;

    RECT rc;
    rc.top = 0;
    rc.left = 0;
    rc.right = static_cast<LONG>(windowSize.width);
    rc.bottom = static_cast<LONG>(windowSize.height);
