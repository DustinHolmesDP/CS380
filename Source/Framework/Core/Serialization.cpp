    ...
    rapidjson::MemoryPoolAllocator<> &allocator)
{
    ...
    rj::Value distanceValue;
    distanceValue.SetInt(data.distance);
    ...
    val.AddMember("Distance", distanceValue, allocator);
...
bool Serialization::deserialize_from(rapidjson::Value &val, PathingTestData &data, const std::wstring &id)
{
    ...
    auto distIter = val.FindMember("Distance");
    ...
    if (extract_int(distIter, end, data.distance, prefix, L"Distance", id) == false)
    {
        valid = false;
    }
