...
void ProjectTwo::build_ui()
{
    ...
    Callback movementCB = std::bind(&AStarAgent::toggle_follow_path, agent);
    Getter<bool> movementGet = std::bind(&AStarAgent::get_follow_path, agent);
    auto movementButton = ui->create_toggle_button(UIAnchor::BOTTOM, singleButton,
        10, movementCB, L"Movement", movementGet);
    ...
    auto debugButton = ui->create_toggle_button(UIAnchor::BOTTOM, movementButton,
...
void ProjectTwo::on_left_mouse_click()
{
    ...
    if (worldPos.second == true)
    {
        ...
        if (terrain->is_valid_grid_position(gridPos) == true)
        {
